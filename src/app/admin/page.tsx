...
export default async function AdminDashboard() {
  await requireOwner();
  const d = await getBusinessDashboard();
  ...
  return (
    <div className="min-h-screen bg-hethu-cream">
      <PageHeader title={businessConfig.name} subtitle={weekDay} />
      ...
      <div className="space-y-4 p-4">
        ...
        <section className="rounded-2xl bg-white p-4 shadow-sm">
          <div className="flex items-center justify-between">
            ...
            <Link href="/admin/orders" className="text-xs font-bold text-hethu-red">
              See all
            </Link>
          </div>
        ...
        <section>
          ...
          <div className="grid grid-cols-2 gap-2">
            <QuickAction href="/admin/sales" label="Record sale" />
            <QuickAction href="/admin/consignment" label="Give stock" />
            <QuickAction href="/admin/menu/new" label="Add product" />
            <QuickAction href="/admin/qr" label="Order QR code" />
          </div>
        </section>
      </div>
    </div>
